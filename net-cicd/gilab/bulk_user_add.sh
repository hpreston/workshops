...
printf "Creating user 'user${i}' ..."
create_gitlab_token 2>&1 >> gitlab_setup.log
curl -s --header "PRIVATE-TOKEN: $personal_access_token" -d "email=user${i}@devnetsandbox.cisco.com&password=cisco1234&username=user${i}&name=user${i}&skip_confirmation=true" "http://10.10.20.20/api/v4/users" 2>&1 >> gitlab_setup.log
success
done
